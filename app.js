...
app.post('/api/chatbot', async (req, res) => {
    ...
    try {
        ...
        // Filter out any messages that are null or have an empty message content to avoid errors
        const filteredMessages = conversation
            .filter((chat) => chat.msg && chat.msg.trim())
            .map((chat) => ({
        ...
        const response = await openai.chat.completions.create({
            model: "gpt-3.5-turbo", // Use the latest GPT model - Can also use GPT-4
            // Explain the system's role and provide the conversation history
            messages: [
